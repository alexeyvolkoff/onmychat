...
Крім того, система періодично виконує автоіндексацію файлів у вашому сховищі. Єдиний індекс семантичного пошуку (RAG і пошук використовують одну базу) зберігається в unified_index:

```bash
ls /opt/onmychat/memory_index/unified_index/
# Має бути колекція 'omd_unified'
```
