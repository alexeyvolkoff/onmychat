...
Кроме того, система периодически производит автоиндексацию файлов в вашем хранилище. Единый индекс семантического поиска (RAG и поиск используют одну базу) сохраняется в unified_index:

```bash
ls /opt/onmychat/memory_index/unified_index/
# Должна быть коллекция 'omd_unified'
```
